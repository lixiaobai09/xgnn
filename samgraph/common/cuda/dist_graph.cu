...
std::vector<DistGraph::GroupConfig> PartitionSolver::solve() const  {
  ...
  std::vector<std::tuple<int, int>> asc_dgree_gpu_order(num_ctx);
  for (int i = 0; i < num_ctx; ++i) {
    ...
    asc_dgree_gpu_order[i] = std::make_tuple(
        i, static_cast<int>(neighbor_adjacency[i].size()));
  }
  // sort nodes by ascending order to iterate
  std::sort(asc_dgree_gpu_order.begin(), asc_dgree_gpu_order.end(),
      [](auto x, auto y) {
  ...
  for (auto item : asc_dgree_gpu_order) {
