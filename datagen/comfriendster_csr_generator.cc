...
class Timer {
 public:
  ...
  double Passed() const {
    const auto elapsed = std::chrono::duration_cast<std::chrono::duration<double>>(std::chrono::steady_clock::now() - _start_time);
...
std::string output_dir = raw_data_dir;
...
struct ThreadCtx {
  ...
  ThreadCtx(int thread_idx, size_t v_sz, size_t e_sz) : thread_idx(thread_idx), v_cnt(0), e_cnt(0) {
    v_list.reserve(v_sz);
    e_list.reserve(e_sz);
  }
};
...
int main() {
  ...
  size_t vfile_partition_nbytes = (raw_graph.vfile.nbytes + num_threads - 1) / num_threads;
  size_t efile_partition_nbytes = (raw_graph.efile.nbytes + num_threads - 1) / num_threads;
  ...
  for (size_t i = 0; i < num_threads; i++) {
    ...
    threads.emplace_back(threadLoadGraph, std::ref(*thread_ctx[i]), std::ref(raw_graph), v_start, v_end, e_start, e_end);
  }
  ...
  for (size_t i = 0; i < num_threads; i++) {
    threads.emplace_back(threadPopulateHashtable, std::ref(*thread_ctx[i]), std::ref(o2n_hashtable), v_cnt_prefix_sum[i]);
  }
  ...
  for (size_t i = 0; i < num_threads; i++) {
    threads.emplace_back(threadMapEdges, std::ref(*thread_ctx[i]), std::ref(o2n_hashtable), std::ref(new_edge_list), e_cnt_prefix_sum[i]);
  }
